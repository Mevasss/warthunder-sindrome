...
{
	return CUnit(memory->read<std::uintptr_t>(base_address + offsets::_CPlayer::player_unit));
}
...
{
	return GuiState(memory->read<uint8_t>(this->base_address + offsets::_CPlayer::gui_state));
}

auto CPlayer::name() -> std::string
{
	return memory->read_string(this->base_address + offsets::_CPlayer::name);
}
